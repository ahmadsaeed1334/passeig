...
@section('content')
								...
								<tbody>
									@forelse($blogTitles as $index => $blogTitle)
										<tr>
											...
											<td>
												<a href="{{ route('blogs.editTitle', $blogTitle->id) }}"
													class="btn btn-icon btn-light btn-active-light-{{ primary_color() }} btn-sm mr-3">
									...
									@forelse($blogs as $index => $blog)
										<tr>
											...
											<td>
												...
												<button type="button"
													class="btn btn-icon btn-light btn-active-light-{{ primary_color() }} btn-sm view-comments mr-3"
													data-blog-id="{{ $blog->id }}">
													<i class="fa-solid fa-message fs-6 fw-bold fw-bolder"></i>
												</button>
	...
	<!-- Include jQuery in the <head> section or before your script -->
	...
	<script>
		$(document).ready(function() {
			$('.view-comments').on('click', function() {
				...
				var modalContent = $('#commentsContent');

				// Set the title of the modal dynamically (optional)
				$('#commentsModalLabel').text('Comments for Blog ID ' + blogId);

				// Show the modal
				modal.modal('show');

				// Generate the URL using the route helper in Blade
				var url = '{{ route('blogs.loadComments', ':id') }}';
				...
				$.ajax({
					...
					error: function() {
						modalContent.html('<p>An error occurred while loading comments.</p>');
					}
				});
			});
		});
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="replyForm">
						@csrf
						<!-- This hidden field should contain the correct blog_id -->
						<input type="hidden" name="blog_id" id="replyBlogId" value="{{ $blog->id }}">
						<input type="hidden" name="parent_id" id="replyParentId">
						<div class="mb-3">
							<label for="replyBody" class="form-label">Your Reply</label>
							<textarea class="form-control" id="replyBody" name="body" rows="3"></textarea>
						</div>
