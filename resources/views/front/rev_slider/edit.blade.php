        ...
        <div id="kt_app_content" class="app-content flex-column-fluid">
            <div id="kt_app_content_container" class="app-container container{{ general('layout') }}">
                <div class="card card-flush">
                    ...
                    <form action="{{ route('rev_slider.update', $rev_slider->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group ms-3 me-3 mt-3">
                            <label class="mb-3 ms-3 required" for="title">Title</label>
                            <input type="text" class="form-control" id="title" name="title" value="{{ old('title', $rev_slider->title) }}">
                        </div>
                        ...
                        <div class="form-group ms-3 me-3 mt-3">
                            <div class="col-lg-4">
                                ...
                                <div class="image-input image-input-outline align-items-center" style="background-image: url({{ asset('img/bg-back.jpg') }}); background-size:100% 100%;width:300px">
                                    <div class="image-input-wrapper" id="image-preview" style="background-image: url({{ $rev_slider->image ? asset('storage/'.$rev_slider->image) : asset('img/bg-back.jpg') }}); background-size:100% 100%;width:300px">
                                    </div>
                        ...
                        <div class="form-group ms-3 me-3 mt-3">
                            ...
                            <textarea class="form-control" id="long_description" name="description" rows="5">{{ old('description', $rev_slider->description) }}</textarea>
                        </div>
                        <button type="submit" class="btn btn-primary ms-3 mt-3 mb-5">Update</button>
                        <a href="{{ route('rev_slider.index') }}" class="btn btn-secondary ms-5 mt-3 mb-5">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
