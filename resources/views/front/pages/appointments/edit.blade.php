                ...
                <div class="card card-flush">
                    ...
                    <form action="{{ route('appointments.update', $appointment->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group ms-3 me-3 mt-3">
                            ...
                            <input type="text" class="form-control" id="title" name="title" value="{{ old('title', $appointment->title) }}">
                        </div>
                        <div class="form-group ms-3 me-3 mt-3">
                            <label class="mb-3 ms-3 required" for="long_description">Long Description</label>
                            <textarea class="form-control" id="long_description" name="long_description" rows="5">{{ old('long_description', $appointment->long_description) }}</textarea>
                        </div>

                        <div class="form-group ms-3 me-3 mt-3">
                            <label class="mb-3 ms-3 required" for="button">Button</label>
                            <input type="text" class="form-control" id="button" name="button" value="{{ old('button', $appointment->button) }}">
                        </div>

                        <button type="submit" class="btn btn-primary ms-3 mt-3 mb-5">Update</button>
                        <a href="{{ route('appointments.index') }}" class="btn btn-secondary ms-5 mt-3 mb-5">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
