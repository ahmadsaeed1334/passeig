                ...
                <div class="card card-flush">
                    <div class="card-header mt-6">
                        <div class="card-title">
                            <div class="d-flex align-items-center position-relative me-5 my-1">
                                ...
                                <input wire:model.debounce.500ms="search" type="text" data-kt-Users-table-filter="search"
                                    ...
                                    placeholder="Search {{ Str::singular($page_title ?? ' Services') }} by id or name" />
                            </div>
                        </div>
                        <div class="card-toolbar">
                            ...
                            {{-- <a type="button" class="btn btn-light-{{ primary_color() }}" href="{{ route('our-services.titlecreate') }}">
                                <span class="svg-icon svg-icon-3">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <rect opacity="0.3" x="2" y="2" width="20" height="20" rx="5" fill="currentColor" />
                                        <rect x="10.8891" y="17.8033" width="12" height="2" rx="1"
                        ...
                        <table class="table-row-dashed fs-6 gy-5 mb-0 table align-middle" id="kt_HeroBanners_table">
                                ...
                                <tbody>
                                    @foreach($servicesTitles as $servicesTitle)
                                        <tr>
                                            ...
                                            <td>{{ \Illuminate\Support\Str::words(strip_tags($servicesTitle->long_description), 10, '...') }}</td>
                                            <td>
                                                ...
                                                {{-- <a href="{{ route('our-services.titleedit', $servicesTitle->id) }}" class="btn btn-sm btn-primary">Edit</a> --}}
                                                {{-- <form action="{{ route('our-services.titledestroy', $servicesTitle->id) }}" method="POST" style="display:inline-block;">
                                                    @csrf
                                                    @method('DELETE')
                            ...
                            <tbody>
                                @foreach($services as $service)
                                    <tr>
                                        <td>{{ $service->title }}</td>
                                        {{-- <td>{{ \Illuminate\Support\Str::words(strip_tags($service->short_description), 10, '...') }}</td> --}}
                                        ...
                                        {{-- <td>{{ \Illuminate\Support\Str::limit($service->long_description, 100) }}</td> --}}
