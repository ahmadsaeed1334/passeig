        ...
        <div id="kt_app_content" class="app-content flex-column-fluid">
            <div id="kt_app_content_container" class="app-container container{{ general('layout') }}">
                <div class="card card-flush">
                    ...
                    <form action="{{ route('provides.update', $provide->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group ms-3 me-3 mt-3">
                            <label class="mb-3 ms-3 required" for="top_title">Top Title</label>
                            <input type="text" class="form-control" id="top_title" name="top_title" value="{{ $provide->top_title }}">
                        ...
                        <div class="form-group ms-3 me-3 mt-3">
                            ...
                            <input type="text" class="form-control" id="short_description" name="short_description" value="{{ $provide->short_description }}">
                            @error('short_description') <span class="text-danger">{{ $message }}</span> @enderror
                        </div>
                        <div class="form-group ms-3 me-3 mt-3">
                            <label class="mb-3 ms-3 required" for="long_description">Long Description</label>
                            <textarea class="form-control" id="long_description" name="long_description" rows="5">{{ $provide->long_description }}</textarea>
                        ...
                        <div class="form-group ms-3 me-3 mt-3">
                            ...
                            @if ($provide->image)
                            <img src="{{ asset('storage/' . $provide->image) }}" alt="{{ $provide->title }}" width="100">
                            @endif
