	...
	<div class="card-header mt-6">
		...
		<div class="card-title">
			...
			<div class="d-flex align-items-center position-relative me-5 my-1">
				...
				<!--end::Svg Icon-->
				<input wire:model.debounce.500ms="search" type="text" data-kt-permissions-table-filter="search"
		...
		<div class="card-toolbar">
			...
			<button type="button" class="btn btn-light-{{ primary_color() }}" data-bs-toggle="modal"
				data-bs-target="#addPermissionsModel">
			...
			<tbody class="fw-semibold text-gray-600">
				@forelse ($permissions as $index => $permission)
					<tr>
						...
						<!--begin::Name=-->
						<td>
							<span wire:click="$dispatch('PermissionID','{{ $permission->id }}')" {!! show_toltip('View Permission Details') !!}
								class="text-dark font-weight-bolder text-hover-{{ primary_color() }} pointer mb-1">
								{{ $permission->name }}
							</span>
						...
						<td>
							...
							@forelse ($permission->roles as $role)
								<span wire:click.prevent="$dispatch('RoleID', '{{ $role->id }}')"
									class="badge badge-light-{{ random_color() }} fs-7 pointer m-1"
									{!! show_toltip('View Role Details') !!}>{{ $role->name }}</span>
							@empty
						...
						<!--end::Assigned to=-->
						<!--begin::Created Date-->
						<td class="text-dark">{{ custom_datetime_format($permission->created_at) }}</td>
						<!--end::Created Date-->
						<!--begin::Action=-->
						<td class="text-end">
							<!--begin::Update-->
							<button wire:click="edit('{{ $permission->id }}')"
								class="btn btn-icon btn-active-light-{{ primary_color() }} w-30px h-30px me-3" data-bs-toggle="modal"
								data-bs-target="#addPermissionsModel">
