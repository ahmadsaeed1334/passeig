<x-app-layout>
	...
	<form class="form w-100" method="POST" action="{{ route('login') }}">
		...
		<div class="mb-10 text-center">
			<!--begin::Title-->
			<h1 class="text-dark mb-3">{{ __('Sign In to') }} <span
					class="text-{{ primary_color() }}">{{ setting('general_settings.app_name') }}</span>
			</h1>
			<!--end::Title-->
			<!--begin::Link-->
		...
		<!--begin::Heading-->
		<x-jet-validation-errors class="rounded-0 mb-3" />
		...
		<div class="fv-row mb-10">
			...
			<!--end::Wrapper-->
			<!--begin::Input-->
			<input class="form-control form-control-lg form-control-solid {{ $errors->has('password') ? ' is-invalid' : '' }}"
		...
		<div class="text-center">
			<!--begin::Submit button-->
			<button type="submit" class="btn btn-lg btn-{{ primary_color() }} w-100 mb-5">
